...
| **DB connection refused** | Database service is not initialized correctly or using incorrect `DB_HOST` | Run `docker compose ps` to check if the `db` container is persistently healthy. Verify `DB_HOST` in `.env` inside Docker is targeting the internal service name (e.g., `db`) and not `localhost`. |
